          ...
          <div className="relative z-0 max-w-screen-lg mx-auto overflow-hidden lg:rounded-lg aspect-video">
            {imageProps && (
              <Image
                ...
                alt={post.mainImage?.attributes.name || "Thumbnail"}
                placeholder="blur"
                layout="fill"
                loading="eager"
                objectFit="cover"
...
export async function getStaticProps({ params, preview = false }) {
  const post = await httpService.get<PostProp>(`/api/posts/${params.slug || ""}`)
    .then(r => r.data);
  const siteConfig = await httpService.get("/api/site-config").then(r => r.data);
